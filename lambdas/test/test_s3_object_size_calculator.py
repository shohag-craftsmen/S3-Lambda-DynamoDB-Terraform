...
import s3_object_size_calculator


class S3ObjectSizeCalculator(unittest.TestCase):
    ...
    def test_event_parser(self):
        s3_object_count_map = s3_object_size_calculator.event_parser(self.s3_objects)
        self.assertEqual(len(s3_object_count_map.items()), 1)
        self.assertEqual(s3_object_count_map, self.parser_result)
        empty_result = s3_object_size_calculator.event_parser([])
